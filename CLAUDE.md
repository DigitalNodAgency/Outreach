...
- **Current focus:** DISCOVERY DEDUP-AWARE FETCH (v16 | 2026-07-07). The 07-06 scheduled run pulled 50/50 dupes (ICP repo-var edits re-keyed the filter-hash cursor to 0, re-walking already-scraped pool records; enrichment credits wasted on all 50). Fixed on branch `fix/dedup-aware-discovery`: known (name, company) pairs skipped at fetch time pre-enrichment; run keeps walking until MAX_LEADS_PER_RUN NEW leads (scan cap max(target*6, 300)); `max_leads` dispatch input added for tests. After merge: reset current Discovery State offset to 0 + dispatch with max_leads=10 to prove the skip path. BillionVerify (v12) still pending first live field-shape confirmation.
- **Pending decisions:** Merge `fix/dedup-aware-discovery` to main, then run the 10-lead remote test. Confirm no account-level Brevo footer double-ups with the baked-in template footer.
...
- [v16 | 2026-07-07 | DISCOVERY MADE DEDUP-AWARE — the 2026-07-06 scheduled Phase 1 run yielded New: 0, Dupes: 50: the fetch loop consumed exactly `target` records from the cursor offset and deduped only AFTER per-prospect email enrichment, so a cursor reset onto already-scraped pool records burned the whole run budget + 50 enrichment calls for zero new leads. Root cause of the reset CONFIRMED via the live Discovery State tab (3 rows with 3 DIFFERENT filter_keys: 06-27 offset 30, 07-03 offset 50, 07-06 offset 50): the cursor key is a hash of the resolved ICP filters, so Rizan's ICP repo-var edits between runs re-keyed the walk back to offset 0 (by design, pivot-safe) — the 07-06 run then re-fetched pool records 0-49 that the 07-03 manual dispatch had already written. Fix: (1) sheets_client.get_all_name_company_pairs() — (name, company) pairs for EVERY lead row (the existing get_existing_name_company_pairs covers only email-less rows; raw prospects have no email pre-enrichment, so name+company is the only fetch-time key); (2) _fetch_prospects(api_key, target, known_pairs) now walks the pool per-record and skips prospects whose pair is already in the sheet (or already taken this run) BEFORE enrichment, continuing until `target` NEW prospects are collected — bounded by scan_cap = max(target*6, 300) (_SCAN_CAP_MULT/_SCAN_CAP_MIN) to cap page-fetch spend and guard infinite loops; the cursor advances past everything scanned (kept + skipped) so dupes are never re-scanned; returns (prospects, skipped_known); (3) "all scanned prospects already known" logs INFO instead of a pipeline error; fetch-skips fold into stats dupes_skipped (summary email unchanged); post-enrichment _deduplicate untouched (safety net for email-exact + fuzzy-domain); (4) phase1-discovery.yml gained a workflow_dispatch input `max_leads` that overrides MAX_LEADS_PER_RUN for one-off test runs (scheduled runs unaffected — empty inputs fall back to the repo var, and _int_env handles empty-string injection). SEMANTICS CHANGE: MAX_LEADS_PER_RUN now means NEW leads targeted per run, not records fetched. Verified offline via monkeypatched harness (skip / scan-cap / wrap / cursor-offset, 13 checks PASS) + py_compile. Remote proof after merge: reset the current filter_key's Discovery State offset to 0, dispatch with max_leads=10 → expect ~50-60 fetch-skips and 10 new leads written.]
