...
- **Current focus:** ICP SIZE NARROWED TO MID-ONLY (v18 | 2026-07-09). Mohit explicitly wants ONLY mid-sized agencies — repo var `ICP_COMPANY_SIZE` goes `11-200` → `11-50` (drops the 51-200 bucket; matches his written 1-50 spec minus the 1-10 micro bucket). Repo-var-only change, zero code (v17 design). Pool at the narrowed filter ≈ half of before (260k vs 514k prospects pre-seniority; plenty). Existing 51-200 leads in the sheet CONTINUE their sequence (Rizan's call — only new discovery narrows). Cursor auto-resets credit-free on the var flip (filter key 9dc2ff81 → 236f1837, harness-proven). v16 dedup fetch + v17 strict filter both merged and live; BillionVerify (v12) still pending first live field-shape confirmation.
- **Pending decisions:** Rizan flips repo var `ICP_COMPANY_SIZE=11-50` (floor MUST stay 11 — `10-50` re-includes the 1-10 micro bucket) + merges the `docs/icp-mid-size-only` branch. Confirm no account-level Brevo footer double-ups with the baked-in template footer.
