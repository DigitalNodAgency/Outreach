...
- **Current focus:** EMAIL COPY SWAP (v15 | 2026-07-03). Client delivered a revised 5-touch PR white-label cold-email sequence — rewrote touch-standard-1..5.txt (new touch 5, no breakup, all follow-ups Re:-threaded off Touch 1), DELETED the 4 `templates/variants/*.json` so copy sends verbatim (variation engine was overriding the .txt), bumped config MAX_FOLLOWUPS default 4→5. Rizan already set the `MAX_FOLLOWUPS=5` repo var; footer is email-only by his call (no postal address); existing mid-sequence leads CONTINUE on new copy (no reset). Deploy = merge to main; effective next Phase 2 run (cron 10:17 UTC). BillionVerify (v12) still pending first live field-shape confirmation.
- **Pending decisions:** Merge the v15 email-copy branch to main (schedules/sends only pick up new copy from default branch). Confirm no account-level Brevo footer double-ups with the baked-in template footer.
