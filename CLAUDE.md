...
- **Current focus:** PHASE 2 TIMEOUT FIX (v20 | 2026-07-10). Night run 2026-07-09 hard-killed at `timeout-minutes: 60` (50/day × 100-120s pacing ≈ 92 min). Fix = one PHASE2_TIMEOUT_MINUTES knob (default 180) driving hard kill + engine soft stop; self-governing cap (budget ÷ max gap) so the ramp never outgrows the window; pacing moved AFTER Sheet writes (duplicate-send window closed); budget stop = exit 0 + summary line. PARALLEL track: external scheduler agreed — cron-job.org dispatches phase2 (10:17 UTC) + reply-logger (11:00 UTC); after a 3-4 day parallel window, remove those two `schedule:` blocks. Prior: NON-US LEAD LEAK FIXED (v19 | 2026-07-09). The 2026-07-09 run wrote 2 leads with contacts in Taiwan/Israel — the geo filter only ever constrained the COMPANY's location, never the person's. Fix = prospect-level `country_code` filter derived from ICP_REGIONS + credit-free post-fetch `country_name` screen; both foreign leads deleted from the sheet before any send. v18 CONFIRMED LIVE from the Discovery State tab: today's run used key 236f1837 (11-50 + strict linkedin_category, pool 30,816) — Rizan's var flip is in effect. BillionVerify (v12) still pending first live field-shape confirmation.
- **Pending decisions:** Merge `fix/us-only-prospect-filter` before the Mon 2026-07-13 07:17 UTC Phase 1 run (else the next run can leak more non-US contacts). Confirm no account-level Brevo footer double-ups with the baked-in template footer.
...
> Geo is filtered at BOTH levels (v19): company region codes AND prospect-level
> `country_code` (accepts bare `us`) — the company filter alone admits US companies whose
> contact lives abroad. A credit-free post-fetch screen on prospect `country_name` backs it up.
